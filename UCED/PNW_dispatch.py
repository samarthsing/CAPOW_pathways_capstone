...
model.bat_dis_on = Var(model.Zone5Battery,model.HH_periods, within=Binary, initialize=0)
model.bat_charge_on = Var(model.Zone5Battery,model.HH_periods, within=Binary, initialize=0)
...
def Zone5_Balance(model,i):
    ...
    return s1 + s2 + s3 + other + imports - exports + discharge >= model.HorizonDemand['PNW',i] + store

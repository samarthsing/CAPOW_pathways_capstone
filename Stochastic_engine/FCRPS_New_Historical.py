...
def simulate(sim_years,job_id):
    ...
    max_df=pd.read_csv("months_max.csv")
    max_df.columns=[0,1,2,3,5,7,9,11,12]
    ...
    #job_id=456
    d=pd.read_csv('Synthetic_streamflows/synthetic_streamflows_FCRPS_{}.csv'.format(job_id),header=None)
    ...
    #d.to_csv("PNW_hydro/FCRPS/streamflows.txt",sep=" ",header=False)
    no_days = int(len(d))
    no_years = int(no_days/365)
    
    
    #historical_discharge=pd.read_csv(r"C:\Users\ss9vz\Downloads\multi_objective\discharges_historical_94_16_hyssr.csv",sep=",",header=None)
    #historical_storage=pd.read_csv(r"C:\Users\ss9vz\Downloads\multi_objective\storage_historical_94_16_hyssr.csv",sep=",",header=None)
    #no_years = 100
    #no_days = no_years*365
    #list_ind=[3952,3953,3954,7602,7603,7604,13442,13443,13444]
    #list_ind=[3952,3953,3954,18187,18188,18189,18917,18918,18919,25487,25488,25489]
    #list_ind=[7237,7238,7239,24392,24393,24394]
    #list_ind_11=[668,4683,5778,7238,7968,9428,13808,18918,19648,20743,21838,23298,23663,24758,25488,26218,27313,28773,36438]
    calender=pd.read_excel('PNW_hydro/FCRPS/daily_streamflows.xlsx','Calender',header=None)
    calender.to_csv("PNW_hydro/FCRPS/calendar.txt",sep=" ")
    ...
    no_days = len(c)
    years = c[:,3]
    #c.to_csv("PNW_hydro/FCRPS/calendar_100.txt",sep=" ")
    #np.savetxt("PNW_hydro/FCRPS/julians_1188.txt",julians,delimiter=',')
    ...
    #% 2 LIBBY	3
    #% 3 DUNCAN	5 canada
    #% 4 CORRA LINN	6 canada
    #% 15 REVELSTOKE	41 canada
    #% 17 BRILLIANT	50 canada
    #% 46 BONNERS FERRY	400 canada
    ...
    MCD_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Mica_Daily',usecols='B:M',skiprows=3,header=None)#mica, arrow, duncan
    ARD_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Arrow_Daily',usecols='B:G',skiprows=3,header=None)
    LIB_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Libby_Daily',usecols='B:F',skiprows=3,header=None)#bpa
    DNC_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Duncan_Daily',usecols='B:F',skiprows=3,header=None)
    HHO_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='HungryHorse_Daily',usecols='B:I',skiprows=3,header=None)#bpa
    ALB_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Albeni_Daily',usecols='B:C',skiprows=3,header=None)#bpa
    GCL_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='GrandCoulee_Daily',usecols='B:J',skiprows=3,header=None)#bpa
    GCL_fc.to_csv("PNW_hydro/FCRPS/GCL_fc.txt",sep=" ",header=None)
    BRN_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Brownlee_Daily',usecols='B:U',skiprows=3,header=None)
    DWR_fc = pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx',sheet_name='Dworshak_Daily',usecols='B:K',skiprows=3,header=None)#bpa
    ...
    GCL_CRC_hm3.to_csv("PNW_hydro/FCRPS/GCL_CRC_hm3.txt",sep=" ")
    GCL_ARC_hm3 = pd.read_excel('PNW_hydro/FCRPS/ORC.xlsx',sheet_name='GCL_ARC', header=None)
    GCL_ARC_hm3.to_csv("PNW_hydro/FCRPS/GCL_ARC_hm3.txt",sep=" ")
    ...
    s_max = d.iloc[:,1]
    s_max.to_csv("PNW_hydro/FCRPS/max_storage.txt",sep=" ")
    
    #Maximum Hydraulic Capacity
    d=pd.read_excel('PNW_hydro/FCRPS/res_specs2.xlsx','HydraulicMax')
    f_max = d.iloc[:,3]*0.8
    f_max.to_csv("PNW_hydro/FCRPS/max_hydropower.txt",sep=" ")
    #Storage-elevation curves
    d = pd.read_excel('PNW_hydro/FCRPS/res_specs.xlsx','StorEq')
    StorEq = d
    StorEq.to_csv("PNW_hydro/FCRPS/storage_elevation.txt",sep=" ",header=False)
    #Outflow-tailwater elevation curves
    d = pd.read_excel('PNW_hydro/FCRPS/res_specs.xlsx','TailEq')
    TailEq = d
    TailEq.to_csv("PNW_hydro/FCRPS/tailwater_elevation.txt",sep=" ",header=False)
    #Starting storage levels(kAF).
    #Note: model starts August 1, 1928, so reservoirs are assumed full to start.
    s_1= s_max[0]
    s_2=s_max[1]
    s_3=s_max[2]
    #s_3=historical_storage.iloc[0,2]
    s_4=s_max[3]
    s_5=s_max[4]
    s_6=s_max[5]
    #s_6=historical_storage.iloc[0,5]
    s_7=s_max[6]
    s_8=s_max[7]
    s_9=s_max[8]
    s_10=s_max[9]
    #s_10=historical_storage.iloc[0,9]
    s_11=s_max[10]
    s_12=s_max[11] 
    s_13=s_max[12]
    #s_13=historical_storage.iloc[0,12]
    s_14=s_max[13]
    s_15=s_max[14]
    s_16=s_max[15]
    s_17=s_max[16]
    ...
    s_28=s_max[27]
    s_29=s_max[28]
    #s_29=historical_storage.iloc[0,28]
    s_30=s_max[29]
    #s_30=historical_storage.iloc[0,29]
    s_31=s_max[30]
    #s_31=historical_storage.iloc[0,30]
    s_32=s_max[31]
    #s_32=historical_storage.iloc[0,31]
    s_33=s_max[32]
    #s_33=historical_storage.iloc[0,32]
    s_34=s_max[33]
    #s_34=historical_storage.iloc[0,33]
    s_35=s_max[34]
    s_36=s_max[35]
    #s_36=historical_storage.iloc[0,35]
    s_37=s_max[36]
    #s_37=historical_storage.iloc[0,36]
    s_38=s_max[37]
    #s_38=historical_storage.iloc[0,37]
    s_39=s_max[38]
    #s_39=historical_storage.iloc[0,38]
    s_40=s_max[39]
    s_41=s_max[40]
    s_42=s_max[41]
    s_43=s_max[42]
    #s_43=historical_storage.iloc[0,42]
    s_44=s_max[43]
    s_45=s_max[44]
    s_46=s_max[45]
    s_47=s_max[46]
    ...
    #job_id=805
    ICFs=pd.read_csv('PNW_hydro/FCRPS/ICFcal_{}.csv'.format(job_id),header=None)
    ICFs.to_csv('PNW_hydro/FCRPS/ICF.txt',sep=" ",header=None)
    ICFs = np.array(ICFs)
    ...
    output_ARD=pd.DataFrame(columns=["ARD"])
    output_BRN=pd.DataFrame(columns=["BRN"])
    output_DWR=pd.DataFrame(columns=["DWR"])
    #########
    #sfore arrays#
    #########
    ...
    BNN_tail = np.zeros((sim_days,1))
    BNN_stage= np.zeros((sim_days,1))
    ...
    for i in range(0,sim_days):
        ...
        discharge[i,0] = local.iloc[i,0]
        MCD_dg=local.iloc[i,0]*.001987
      ...
      #inflow is RVC_dg + local.iloc[i,1] out flow is VPDR or APDR
        ...
        #convert project inflows ("local") to kAF.
            ...
            if(i in list_ind):
                ...
                print("s_2 before is")
                print(s_2)
                print("s_2 + RVC_dg + local.iloc[i,1]*.001987 - ARD_ORC[julian-1]")
                print(s_2 + RVC_dg + local.iloc[i,1]*.001987 - ARD_ORC[julian-1])
                print("ARD_min*.001987")
                print(ARD_min*.001987)
                print("ARD_dg")
                print(ARD_dg)
            '''
        ...
        if(i in list_ind):
            ...
            print("RVC_dg")
            print(RVC_dg)
            print("local.iloc[i,1]*.001987")
            print(local.iloc[i,1]*.001987)
            print("ARD_ORC[julian-1]")
            print(local.iloc[i,1]*.001987)
            print("s_max[1]")
            print(s_max[1])
            print("ARD_min*.001987")
            print(ARD_min*.001987)
            print("s_2 + RVC_dg + local.iloc[i,1]*.001987 - s_max[1]")
            print(s_2 + RVC_dg + local.iloc[i,1]*.001987 - s_max[1])
            print("overflow")
            print(overflow)
            print("stor_avail")
        ...
        """
        discharge[i,1] = local.iloc[i,1]
        ARD_dg=local.iloc[i,1]*.001987
        ...
        s_2 = s_2 + local.iloc[i,1]*.001987 - ARD_dg
        generation[i,1] = ((1000*52.1208*powerflow[i,1]*14.27641*9.81*.875)/1000000)*24
        storage[i,1] = s_2
        ...
        #threshold).
            ...
            storage[i,3] = s_4
        """
        discharge[i,3] = local.iloc[i,3]
        DNC_dg=local.iloc[i,3]*.001987
        s_4 = s_4 + local.iloc[i,3]*.001987 - DNC_dg
        storage[i,3] = s_4
    ...
    #    FCDend_LIB = np.zeros((years,1))
        ...
        #Flow target
            ...
            LIB_dg=min(LIB_dg,max_df.loc[months[i]-1,2]*.001987)
            #Do flows exceed hydraulic capacity of dam?
            ...
            if overflow>0 and stor_avail<=0:
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
                if s_3 + local.iloc[i,2]*.001987  - LIB_dg > s_max[2]:
                        LIB_dg = s_3 + local.iloc[i,2]*.001987 - s_max[2]
    ...
    ############################################################################
      ...
      ########################################################################
        ...
        #convert project inflows ("local") to kAF. Note: there is a minimum flow release of 3500cfs associated with this dam.
            ...
            HHO_dg=min(HHO_dg,max_df.loc[months[i]-1,5]*.001987)
            ...
            if overflow>0 and stor_avail<=0:
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
                if s_6 + local.iloc[i,5]*.001987  - HHO_dg > s_max[5]:
                        HHO_dg = s_6 + local.iloc[i,5]*.001987 - s_max[5]
    ...
    #    ALB_sfore = np.zeros((sim_days,1))
        ...
        ALB_dg=min(ALB_dg,max_df.loc[months[i]-1,7]*.001987)
            #Do flows exceed hydraulic capacity of dam?
        ...
        if overflow>0 and stor_avail<=0:
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
            if  s_8 + local.iloc[i,7]*.001987 + PSL_dg + CBG_dg - ALB_dg > s_max[7]:
                        ALB_dg = s_8 + local.iloc[i,7]*.001987 + PSL_dg + CBG_dg - s_max[7]
    ...
    #    Storage_left = np.zeros((sim_days,1))
        ...
        #Evacuation mode
            ...
            if i>0:
                ...
                GCL_dg=min(GCL_dg,max_df.loc[months[i]-1,9]*.001987)
            #Do flows exceed hydraulic capacity of dam?
            ...
            #If flows exceed dam's hydraulic capacity.
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
                    if s_10 + local.iloc[i,9]*.001987 + spokane_flow[i-1]*.001987 + discharge[i-1,1]*.001987 + discharge[i-1,17]*.001987 + discharge[i-1,22]*.001987 - GCL_dg > s_max[9]:
                        GCL_dg = s_10 + local.iloc[i,9]*.001987 + spokane_flow[i-1]*.001987 + discharge[i-1,1]*.001987 + discharge[i-1,17]*.001987 + discharge[i-1,22]*.001987 - s_max[9]
            ...
            #If flows exceed dam's hydraulic capacity.
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
                    if s_10 + local.iloc[i,9]*.001987 + spokane_flow[i]*.001987 + discharge[i,1]*.001987 + discharge[i,17]*.001987 + discharge[i,22]*.001987 - GCL_dg > s_max[9]:
                        GCL_dg = s_10 + local.iloc[i,9]*.001987 + spokane_flow[i]*.001987 + discharge[i,1]*.001987 + discharge[i,17]*.001987 + discharge[i,22]*.001987 - s_max[9]
                    powerflow[i,9] = f_max[9]
                    spill[i,9] = GCL_dg - f_max[9]
                elif overflow>0 and stor_avail>0:
                #If there is some storage available, total release must be revised down and additional water
        ...
        #threshold).
            ...
            #If flows exceed dam's hydraulic capacity.
                ...
                if i<286 and i>278:
                    ...
                    print("30000*.001987")
                    print(30000*.001987)
                    print("s_10 + local.iloc[i,9]*.001987 + spokane_flow[i]*.001987 + discharge[i,1]*.001987 + discharge[i,17]*.001987 + discharge[i,22]*.001987 -  GCL_ORC[julian-1]")
                    print(s_10 + local.iloc[i,9]*.001987 + spokane_flow[i]*.001987 + discharge[i,1]*.001987 + discharge[i,17]*.001987 + discharge[i,22]*.001987 -  GCL_ORC[julian-1])
                    print("max_df.loc[months[i]-1,9]*.001987")
                    print(max_df.loc[months[i]-1,9]*.001987)
                    print("GCL_dg")
                    print(GCL_dg)
                    print("GCL_dg - f_max[9]")
                    print(GCL_dg - f_max[9])
                    print("overflow")
                    print(overflow)
                    print("julian")
    ...
    #    Spillheads = np.zeros((sim_days,11))
        ...
        discharge[i,11] = local.iloc[i,11]
        BRN_dg=local.iloc[i,11]*.001987
        ...
        #Evacuation mode
            ...
            DWR_dg=min(DWR_dg,max_df.loc[months[i]-1,12]*.001987)
            #If reservoir is low
            ...
            if overflow>0 and stor_avail<=0:
                ...
                #the same, and anything beyond hydraulic capacity of dam is
                #spilled.
                if  s_13 + local.iloc[i,12]*.001987 - DWR_dg > s_max[12]:
                        DWR_dg = s_13 + local.iloc[i,12]*.001987 - s_max[12]
        ...
        #print(max(0,((1000*BNN_head*powerflow[i,42]*14.27641*9.81*.92)/1000000)*24))
        BNN_tail[i]=BNN_tailwater
        BNN_stage[i]=BNN_forebay
        heads[i,9]=BNN_head
        Spillheads[i,10]=BNN_head
    
        '''   
        temp_ARD=pd.DataFrame(ARD_ORC, columns=["ARD"])
        temp_BRN=pd.DataFrame(BRN_ORC, columns=["BRN"])
        temp_DWR=pd.DataFrame(DWR_ORC, columns=["DWR"])
    
        output_ARD=output_ARD.append(temp_ARD,ignore_index=True)
        output_BRN=output_BRN.append(temp_BRN,ignore_index=True)
        output_DWR=output_DWR.append(temp_DWR,ignore_index=True)
        '''
    ...
    #Data Output
    ############
    np.savetxt('PNW_hydro/FCRPS/discharge_{}.csv'.format(job_id),discharge,delimiter=',')
    np.savetxt('PNW_hydro/FCRPS/storage_{}.csv'.format(job_id),storage,delimiter=',')
    np.savetxt('PNW_hydro/FCRPS/powerflow_{}.csv'.format(job_id),powerflow,delimiter=',')
    np.savetxt('PNW_hydro/FCRPS/spill_{}.csv'.format(job_id),spill,delimiter=',')
    np.savetxt('PNW_hydro/FCRPS/heads_{}.csv'.format(job_id),heads,delimiter=',')
    
    np.savetxt('PNW_hydro/FCRPS/discharge_{}.txt'.format(job_id),discharge,delimiter=' ')
    np.savetxt('PNW_hydro/FCRPS/storage_{}.txt'.format(job_id),storage,delimiter=' ')
    np.savetxt('PNW_hydro/FCRPS/powerflow_{}.txt'.format(job_id),powerflow,delimiter=' ')
    np.savetxt('PNW_hydro/FCRPS/spill_{}.txt'.format(job_id),spill,delimiter=' ')
    np.savetxt('PNW_hydro/FCRPS/heads_{}.txt'.format(job_id),heads,delimiter=' ')
    ...
    #ICFs=pd.DataFrame(ICFs)
    #ICFs.to_csv("PNW_hydro/FCRPS/ICF.txt",sep=" ",header=None)
    '''
    output_ARD.to_csv("ARD_ORC_{}.csv".format(job_id))
    output_BRN.to_csv("BRN_ORC_{}.csv".format(job_id))
    output_DWR.to_csv("DWR_ORC_{}.csv".format(job_id))
    '''
    ...
    df_Willamette = pd.read_excel('Willamette/Output/WillametteDAMS_hydropower_{}.xlsx'.format(job_id), usecols=np.arange(1,9))
    ...
    Total_sum.to_excel('PNW_hydro/PNW_hydro_daily_{}.xlsx'.format(job_id)) #to be used in the dispatch
    np.savetxt('PNW_hydro/FCRPS/BPA_owned_dams_{}.csv'.format(job_id),BPA,delimiter=',') #to be used for BPA revenue
    np.savetxt('PNW_hydro/FCRPS/Path_dams_{}.csv'.format(job_id),Path_dams,delimiter=',') #to be used for path transmission estimation
    np.savetxt('PNW_hydro/FCRPS/Total_PNW_dams_{}.csv'.format(job_id),Total_generation,delimiter=',') #same as dispatch but single dams
    np.savetxt('PNW_hydro/FCRPS/generation_{}.csv'.format(job_id),generation,delimiter=',') #all modeled dams and unmodeled sum

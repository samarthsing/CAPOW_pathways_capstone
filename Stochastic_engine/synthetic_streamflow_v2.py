...
def stream_flow(job_id):
    #job_id=1
    df_temp = pd.read_excel('Synthetic_streamflows/hist_temps_1953_2007.xlsx')
    df_temp=df_temp.iloc[7300:,:].reset_index(drop=True)
    ...
    # Simulation data
    sim_weather=pd.read_csv('Synthetic_weather/synthetic_weather_data_{}.csv'.format(job_id),header=0)
    ...
    BPA_streamflow=pd.read_excel('Synthetic_streamflows/with_added_H_streams_appended.xlsx',header=0)
    ...
    Willamette_streamflow=Willamette_streamflow.iloc[7300:,:].reset_index(drop=True)
    Hoover_streamflow=Hoover_streamflow.iloc[7300:,:].reset_index(drop=True)
    CA_streamflow=CA_streamflow.iloc[7300:,:].reset_index(drop=True)
    BPA_streamflow=BPA_streamflow.iloc[:-3650,:].reset_index(drop=True)
    ...
    df_hist_totals.loc[18,'83L']=df_hist_totals.loc[16,'83L']
    ...
    random.seed(17)
    new_cols = ['Name'] + ['type_' + str(i) for i in range(0,141+4)]
    ...
    Sim_total=T_all[:,:112+4]
    Sim_HDD_CDD=T_all[:,112+4:]
    ...
    ###############################################################################
    # Output
    np.savetxt('Synthetic_streamflows/synthetic_streamflows_FCRPS_{}.csv'.format(job_id),output_BPA,delimiter=',')
    np.savetxt('Synthetic_streamflows/synthetic_streamflows_TDA_{}.csv'.format(job_id),TDA[:,1],delimiter=',')
    np.savetxt('Synthetic_streamflows/synthetic_discharge_Hoover_{}.csv'.format(job_id),output_Hoover,delimiter=',')
    CA=pd.DataFrame(output_CA,columns=name_CA)
    CA.to_csv('Synthetic_streamflows/synthetic_streamflows_CA_{}.csv'.format(job_id))
    Willamatte_Syn=pd.DataFrame(output_WI,columns=name_Will)
    Willamatte_Syn.to_csv('Synthetic_streamflows/synthetic_streamflows_Willamette_{}.csv'.format(job_id))
    ...
    df_leap.to_csv('cord/data/input/forecast_flows_{}.csv'.format(job_id))
